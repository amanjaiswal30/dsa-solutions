...
TOPICS: list[LldTopic] = [
    ...
    LldTopic(
        ...
        [
            ("Log event", [
                "App requests **logger** by hierarchical name (`com.shop.order`).",
                "Logger checks **level** (inherits from parent if unset).",
                "Event fan-out to attached **appenders** (console, file, memory).",
                "If **additive**, parent appenders also receive event.",
            ]),
        ],
        [
